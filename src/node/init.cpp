...
void Shutdown()
{
    ...
    // Shutdown HU Slashing, Signaling, and Finality systems
    hu::ShutdownHuSlashing();
...
// ********************************************************* Step 5: Verify wallet database integrity
    ...
    LogPrintf("BATHRON consensus params: quorum_size=%d, threshold=%d, rotation=%d, leader_timeout=%ds, max_reorg=%d\n",
        consensus.nHuQuorumSize,
        consensus.nHuQuorumThreshold,
        consensus.nHuQuorumRotationBlocks,
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // NEVER wipe btcheadersdb during reindex - it's needed for block 1 SPV validation
                // and will be updated from TX_BTC_HEADERS during block replay
                // EXCEPTION: Wipe on epoch mismatch (genesis reset requires fresh start)
                if (!InitBtcHeadersDB(1 << 20, false, fEpochWipe)) {  // 1 MB cache, wipe only on epoch reset
...
// ********************************************************* Step 8: Backup and Load wallet
    ...
    // Initialize HU Finality, Signaling, and Slashing systems
    hu::InitHuFinality();
    hu::InitHuSignaling();
    hu::InitHuSlashing();
