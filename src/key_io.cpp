...
namespace
{
    ...
    // Address decoding - standard and exchange addresses only
    CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, bool& isExchange)
    {
        ...
        if (DecodeBase58Check(str, data, 23)) {
            ...
            // Unsupported address type (version 63/73) returns CNoDestination
...
// Simplified encoding - only exchange flag
std::string EncodeDestination(const CTxDestination& dest, bool isExchange)
{
    return isExchange ? EncodeDestination(dest, CChainParams::EXCHANGE_ADDRESS) : EncodeDestination(dest, CChainParams::PUBKEY_ADDRESS);
}
...
CTxDestination DecodeDestination(const std::string& str)
{
    ...
    return DecodeDestination(str, Params(), isExchange);
}
...
{
    bool isExchange = false;
    return IsValidDestination(DecodeDestination(str, params, isExchange));
}
