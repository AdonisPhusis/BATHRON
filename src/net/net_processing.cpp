...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    case MSG_TXLOCK_VOTE:
        ...
        return true;
    case MSG_SPORK:
        return true; // BATHRON: Sporks removed - always claim to have it
    case MSG_MASTERNODE_WINNER:
    case MSG_MASTERNODE_ANNOUNCE:
    case MSG_MASTERNODE_PING:
        // BATHRON: Legacy masternode system removed - DMN only
        return false;
    ...
    case MSG_CLSIG:
        return true; // PIVHU: pretend we have it to avoid requests
    }
...
{
    // BATHRON: MSG_SPORK handling removed - sporks no longer exist
    if (inv.type == MSG_SPORK) {
        return false; // No sporks to send
    }

    if (inv.type == MSG_QUORUM_FINAL_COMMITMENT ||
        inv.type == MSG_QUORUM_CONTRIB ||
        inv.type == MSG_QUORUM_COMPLAINT ||
        inv.type == MSG_QUORUM_JUSTIFICATION ||
        inv.type == MSG_QUORUM_PREMATURE_COMMITMENT) {
        return false; // PIVHU: no longer supported
    }

    if (inv.type == MSG_QUORUM_RECOVERED_SIG || inv.type == MSG_CLSIG) {
        return false; // PIVHU: no longer supported
    }
    // nothing was pushed.
    return false;
...
{
    // BATHRON: MSG_SPORK removed - kept for protocol compatibility
    return type == MSG_MASTERNODE_WINNER ||
    ...
    if (strCommand == NetMsgType::ADDR || strCommand == NetMsgType::ADDRV2) {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                // BATHRON: Allowed inv request types while we are in IBD
                // MSG_SPORK removed - sporks no longer exist
    ...
    // HU Finality Signature (BATHRON quorum-based finality)
        ...
        if (std::find(allMessages.begin(), allMessages.end(), strCommand) != allMessages.end()) {
            // BATHRON: Process tier-two messages directly (legacy MN system removed, DMN only)
            {
                ...
                // BATHRON: masternodePayments.ProcessMessageMasternodePayments() removed - DMM handles payments
