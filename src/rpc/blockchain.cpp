...
UniValue getfinalitystatus(const JSONRPCRequest& request)
{
    ...
    result.pushKV("quorum_threshold", consensus.nHuQuorumThreshold);

    // Determine status based on lag
    // Warning threshold = QuorumRotationBlocks, Critical = 2x
    std::string status;
    if (lag <= consensus.nHuQuorumRotationBlocks) {
        status = "healthy";
    } else if (lag <= consensus.nHuQuorumRotationBlocks * 2) {
        status = "lagging";
    } else {
        status = "critical";
    }
...
UniValue getquorum(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getquorum ( height )\n"
            "\nReturns the OPERATOR-BASED quorum for a given block height.\n"
            "\nIn v3.0, quorum is selected from OPERATORS (not MNs).\n"
            "The block producer's operator is EXCLUDED from the quorum.\n"
            "\nArguments:\n"
            "1. height    (numeric, optional) Block height (default: current tip)\n"
            "\nResult:\n"
            "{\n"
            "  \"height\" : n,                    (numeric) Block height\n"
            "  \"cycle_index\" : n,               (numeric) Quorum cycle index\n"
            "  \"quorum_size\" : n,               (numeric) Max operators in quorum\n"
            "  \"quorum_threshold\" : n,          (numeric) Signatures needed for finality\n"
            "  \"producer_operator\" : \"pubkey\", (string) Block producer's operator (excluded)\n"
    ...
    // Get producer's operator (to exclude from quorum)
    CPubKey producerOperator;
    auto scores = mn_consensus::CalculateBlockProducerScores(pindex->pprev, mnList);
    if (!scores.empty()) {
        producerOperator = scores[0].second->pdmnState->pubKeyOperator;
    }

    // Calculate operator-based quorum (v3.0)
    int cycleIndex = hu::GetHuCycleIndex(height, consensus.nHuQuorumRotationBlocks);
    uint256 prevCycleHash = pindex->pprev->GetBlockHash();
    auto quorumOperators = hu::GetHuQuorumOperators(mnList, cycleIndex, prevCycleHash, producerOperator);

    // Get all unique operators with their MN counts
    auto allOperators = hu::GetUniqueOperators(mnList);

    // Count MNs per operator
    ...
    result.pushKV("cycle_index", cycleIndex);
    result.pushKV("quorum_size", consensus.nHuQuorumSize);
    result.pushKV("quorum_threshold", consensus.nHuQuorumThreshold);
    ...
    for (const auto& opKey : quorumOperators) {
