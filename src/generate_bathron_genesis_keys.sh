#!/bin/bash
#
# Generate Testnet Genesis Keys for BATHRON
# =======================================
#
# This script generates 5 key pairs for the testnet genesis block:
#   - MN1_OWNER: Masternode 1 collateral (10,000 BATHRON)
#   - MN2_OWNER: Masternode 2 collateral (10,000 BATHRON)
#   - MN3_OWNER: Masternode 3 collateral (10,000 BATHRON)
#   - DEV_WALLET: Development wallet (50,000,000 BATHRON)
#   - FAUCET: Public faucet (50,000,000 BATHRON)
#
# Output: /tmp/bathron_testnet_genesis_keys.json (NOT in repo!)
#
...
#
# Usage:
#   ./generate_bathron_genesis_keys.sh
#

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BATHROND="$SCRIPT_DIR/bathrond"
CLI="$SCRIPT_DIR/bathron-cli"
DATADIR="/tmp/bathron_keygen_regtest"
OUTPUT_FILE="/tmp/bathron_testnet_genesis_keys.json"

echo "═══════════════════════════════════════════════════════════════════════════"
echo "  BATHRON Testnet Genesis Key Generator"
echo "  (Using regtest for key generation - pubKeyHash is network-agnostic)"
echo "═══════════════════════════════════════════════════════════════════════════"

# Check binaries
if [ ! -x "$BATHROND" ]; then
    echo "ERROR: bathrond not found at $BATHROND"
    echo "Please compile first: cd BATHRON && make -j\$(nproc)"
    exit 1
fi

if [ ! -x "$CLI" ]; then
    echo "ERROR: bathron-cli not found at $CLI"
    exit 1
fi

# Kill any existing regtest processes
pkill -f "bathrond.*keygen" 2>/dev/null || true
sleep 1
...
# Start bathrond in background (regtest mode) with explicit RPC params
"$BATHROND" -regtest -datadir="$DATADIR" -rpcuser=keygen -rpcpassword=keygen123 -rpcport=18998 -daemon -printtoconsole=0
...
echo ""
echo "  Next steps:"
echo "  1. Copy pubKeyHash values to CreateBATHRONTestnetGenesisBlock() in chainparams.cpp"
