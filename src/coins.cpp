...
{
    if (!GetSaplingAnchorAt(currentRoot, tree)) {
        // Anchor not found - this can happen during reorg if anchor wasn't persisted
        // Initialize with empty tree to allow the operation to proceed
        LogPrintf("%s: Sapling anchor %s not found, using empty tree\n", __func__, currentRoot.ToString());
