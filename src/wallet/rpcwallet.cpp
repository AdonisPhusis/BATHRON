    ...
    for (auto it = pwallet->NewAddressBookIterator(); it.IsValid(); it.Next()) {
        ...
        if (addrBook.name == label) {
            if (!addrBook.isShielded()) {
                ret.pushKV(EncodeDestination(*it.GetCTxDestKey(), AddressBook::IsExchangePurpose(addrBook.purpose)), AddressBookDataToJSON(addrBook, false));
            } else {
...
UniValue getnewexchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new BATHRON exchange address for receiving transparent funds only.\n"
            ...
            "\nResult:\n"
            "\"address\"    (string) The new bathron exchange address\n"
...
static UniValue ListaddressesForPurpose(CWallet* const pwallet, const std::string strPurpose)
{
    ...
    if (AddressBook::IsExchangePurpose(strPurpose)) {
...
UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    bool isExchange = false, isShielded = false;
    const CWDestination& dest = Standard::DecodeDestination(request.params[0].get_str(), isExchange, isShielded);
    // Make sure the destination is valid
    if (!Standard::IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    }
...
// NOTE: sendtoaddress REMOVED - use "send piv <addr> <amount>" instead (see rpc_state.cpp)
...
UniValue getbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() > 3))
        throw std::runtime_error(
            ...
            "  \"piv\": n,             (numeric) M0 transparent, available for spending\n"
    ...
    CAmount nPIV = pwallet->GetAvailableBalance(filter, true, nMinDepth);
    ...
    CAmount nTotal = nPIV + nShield;

    UniValue result(UniValue::VOBJ);
    result.pushKV("total", ValueFromAmount(nTotal));
    result.pushKV("m0", ValueFromAmount(nPIV));
...
static UniValue legacy_sendmany(CWallet* const pwallet, const UniValue& sendTo, int nMinDepth, std::string comment, const UniValue& subtractFeeFromAmount)
{
    ...
    for (const std::string& name_ : keys) {
        ...
        CTxDestination dest = DecodeDestination(name_, isExchange);
        if (!IsValidDestination(dest))
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    for (const std::string& key : sendTo.getKeys()) {
        bool isExchange = false, isShielded = false;
        Standard::DecodeDestination(key, isExchange, isShielded);
        if (isShielded) {
            fShieldSend = true;
            break;
        }
...
static UniValue ListReceived(CWallet* const pwallet, const UniValue& params, bool by_label, int nBlockHeight)
{
    ...
    for (auto& itAddrBook = itAddr; itAddrBook.IsValid(); itAddrBook.Next()) {
        ...
        if (by_label) {
            ...
            obj.pushKV("address", EncodeDestination(address, AddressBook::IsExchangePurpose(label)));
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending HU\n"
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "getstakingstatus\n"
            "\nDEPRECATED. Returns a stub response for compatibility.\n"
            "\nBATHRON uses masternode-only consensus - no PoS staking.\n"
            "\nFor chain status, use: getblockchaininfo\n"

            "\nResult:\n"
            "{\n"
            "  \"deprecated\": true,\n"
            "  \"message\": \"MN-only consensus - no staking\"\n"
            "}\n"
    ...
    UniValue obj(UniValue::VOBJ);
    obj.pushKV("deprecated", true);
    obj.pushKV("message", "MN-only consensus - no staking");
