                                                  ...
                                                  bool selectTransparentCoins)
{
    ...
    BOOST_ASSERT_MSG(
            CheckTransaction(operation.getFinalTx(), state),
...
BOOST_AUTO_TEST_CASE(test_in_block_and_mempool_notes_double_spend)
{
    ...
    // HU_COINBASE_MATURITY = 10, so at height 110:
    // - Mature: blocks 1-100 (depth >= 11) = 100 blocks
    // - Immature: blocks 101-110 (depth < 11) = 10 blocks
    BOOST_CHECK_EQUAL(pwalletMain->GetAvailableBalance(), CAmount(250 * COIN * 99)); // 99 blocks available (excluding unconfirmed)
    BOOST_CHECK_EQUAL(pwalletMain->GetImmatureBalance(), CAmount(250 * COIN * 10)); // 10 blocks immature

    // Now that we have the chain, let's shield 100 M0
    // single recipient
    std::vector<SendManyRecipient> recipients;
    ...
    SyncWithValidationInterfaceQueue();
    BOOST_CHECK_EQUAL(pwalletMain->GetAvailableShieldedBalance(), CAmount(100 * COIN)); // 100 shield M0
    BOOST_CHECK_EQUAL(pwalletMain->GetUnconfirmedShieldedBalance(), CAmount(0)); // 0 shield M0
