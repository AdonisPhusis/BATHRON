...
public:
    ...
    explicit CDeterministicMNState(const ProRegPL& pl)
    {
        ...
        pubKeyOperator = pl.pubKeyOperator;  // PIVHU: Direct assignment (ECDSA)
    ...
    {
        pubKeyOperator = CPubKey();  // PIVHU: Reset ECDSA key
        addr = CService();
...
public:
    ...
    CDeterministicMNCPtr GetMNByOperatorKey(const CPubKey& pubKey);  // PIVHU: ECDSA (returns first match)
...
public:
    ...
    // HU: GetAllQuorumMembers removed - see hu/hu_quorum.h for HU quorum
