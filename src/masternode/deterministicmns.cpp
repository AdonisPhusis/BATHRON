...
/**
 * BATHRON: Inject genesis masternodes into DMN list at block 0.
...
static void InjectGenesisMasternodes(CDeterministicMNList& mnList, const uint256& blockHash)
{
    ...
    for (const auto& genesisMN : consensusParams.genesisMNs) {
        try {
            ...
            // The real collateral (10k BATHRON) is created at block 1 premine
...
std::string CDeterministicMNState::ToString() const
{
    ...
    return strprintf("CDeterministicMNState(nRegisteredHeight=%d, nLastPaidHeight=%d, nPoSePenalty=%d, nPoSeRevivedHeight=%d, nPoSeBanHeight=%d, nRevocationReason=%d, ownerAddress=%s, operatorPubKey=%s, votingAddress=%s, addr=%s, payoutAddress=%s, operatorPayoutAddress=%s)",
        ...
        EncodeDestination(keyIDOwner), HexStr(pubKeyOperator), EncodeDestination(keyIDVoting), addr.ToStringIPPort(), payoutAddress, operatorPayoutAddress);
}
...
// PIVHU: ECDSA operator key lookup (returns first match - legacy)
CDeterministicMNCPtr CDeterministicMNList::GetMNByOperatorKey(const CPubKey& pubKey)
{
...
bool CDeterministicMNManager::BuildNewListFromBlock(const CBlock& block, const CBlockIndex* pindexPrev, CValidationState& _state, CDeterministicMNList& mnListRet, bool debugLogs)
{
    ...
    oldList.ForEachMN(false, [&](const CDeterministicMNCPtr& dmn) {
        ...
        // BATHRON: Genesis MNs (registered at height 0) are confirmed immediately at block 1
        ...
        bool bBootstrapMN = (dmn->pdmnState->nRegisteredHeight <= BATHRON_BOOTSTRAP_HEIGHT);
    ...
    for (int i = 1; i < (int)block.vtx.size(); i++) {
        ...
        if (tx.nType == CTransaction::TxType::PROREG) {
            ...
            if (newState->nPoSeBanHeight != -1) {
                // PIVHU: only revive when all keys are set (ECDSA)
            ...
            // }
            auto newState = std::make_shared<CDeterministicMNState>(*dmn->pdmnState);
            // PIVHU: ECDSA pubkey comparison (direct, no .Get())
            if (newState->pubKeyOperator != pl.pubKeyOperator) {
                // reset all operator related fields and put MN into PoSe-banned state in case the operator key changes
                newState->ResetOperatorFields();
                newState->BanIfNotBanned(nHeight);
            }
            // PIVHU: Direct assignment for CPubKey (no .Set())
    ...
    if (nHeight > consensus.nDMMBootstrapHeight && pindexPrev != nullptr) {
        ...
        if (producerSlot > 0) {
            ...
            // Penalize all MNs that missed their slot (0 to producerSlot-1)
            for (int i = 0; i < producerSlot && i < (int)scores.size(); i++) {
...
CDeterministicMNList CDeterministicMNManager::GetListForBlock(const CBlockIndex* pindex)
{
    ...
    while (true) {
        ...
        if (!evoDb.Read(std::make_pair(DB_LIST_DIFF, pindex->GetBlockHash()), diff)) {
            ...
            if (!isValidEmptyList) {
                // CRITICAL FIX: During initial sync, blocks may arrive out of order.
                // If evodb data is missing for a block that hasn't been connected yet,
                // return an empty list instead of throwing. The caller can check if the
                // list is empty and handle accordingly.
                // Only throw if we're NOT during initial sync (indicates corruption).
                ...
                // During IBD, return empty list - caller should skip MN-related checks
                LogPrintf("GetListForBlock: evodb data not yet available for block %s at height %d (IBD)\n",
            ...
            // BATHRON: Inject genesis masternodes at block 0 to bootstrap DMM
...
{
    return true;  // HU: No legacy masternode system, DMN only from genesis
}
