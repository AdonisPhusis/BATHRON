extends Node
## PIV2 Daemon Manager - Auto-launch and manage piv2d lifecycle
...
func _setup_paths() -> void:
	...
	if exe_dir.ends_with("MacOS"):
		# Inside .app bundle: PIV2-Wallet.app/Contents/MacOS/
		_daemon_path = exe_dir.path_join("piv2d")
	else:
	...
	if not FileAccess.file_exists(_daemon_path):
		var fallbacks := [
			"/usr/local/bin/piv2d",
			"/opt/homebrew/bin/piv2d",
			OS.get_environment("HOME") + "/piv2d",
			OS.get_environment("HOME") + "/PIV2-Core/src/piv2d",
		]
	...
	# Data directory
	_data_dir = OS.get_environment("HOME") + "/.piv2"

	print("[DaemonManager] Daemon path: ", _daemon_path)
	print("[DaemonManager] Data dir: ", _data_dir)

func _ensure_config() -> void:
	## Create piv2.conf with default seeds if it doesn't exist
	var config_path := _data_dir + "/piv2.conf"
	...
	if not FileAccess.file_exists(config_path):
		var config := """# PIV2 Configuration (auto-generated by wallet)
server=1
rpcuser=walletuser
...
func start_daemon() -> bool:
	## Start piv2d if not already running
	...
	if not FileAccess.file_exists(_daemon_path):
		var msg := "piv2d not found at: " + _daemon_path
		push_error(msg)
...
func stop_daemon() -> bool:
	...
	# Try graceful stop via piv2-cli
	var cli_path := _daemon_path.replace("piv2d", "piv2-cli")
...
func is_daemon_running() -> bool:
	## Check if piv2d is running (by checking lock file or process)
