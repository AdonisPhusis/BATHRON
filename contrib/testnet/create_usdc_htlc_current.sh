...
# Swap: full_b7db4354398b4b38
# ADJUSTED: 0.5 USDC (available balance)

SWAP_ID="full_b7db4354398b4b38"
SECRET="1b78bc7ec39f7f10852068bd3d5e7213566b8a6321aa739d133290aa9bcb0462"
HASHLOCK="b3b4c2087ad8d258a5f589dac34ec896425c44d84f4041030eeaa72e04454cb7"
HTLC_CONTRACT="0xBCf3eeb42629143A1B29d9542fad0E54a04dBFD2"
RECEIVER="0xB6bc96842f6085a949b8433dc6316844c32Cba63"
...
echo "Amount: $AMOUNT_USDC USDC (ADJUSTED)"
echo "Timelock: $TIMELOCK_SECONDS seconds"
echo ""

# Create standalone Python script
cat > /tmp/create_usdc_htlc_standalone.py << 'PYTHON_EOF'
#!/usr/bin/env python3
"""
Create USDC HTLC on Base Sepolia - Standalone version.
...
# Deploy and execute on OP3
echo "Deploying script to OP3..."
scp -i ~/.ssh/id_ed25519_vps /tmp/create_usdc_htlc_standalone.py ubuntu@51.75.31.44:/tmp/

echo "Executing HTLC creation on OP3..."
ssh -i ~/.ssh/id_ed25519_vps ubuntu@51.75.31.44 'python3 /tmp/create_usdc_htlc_standalone.py' | tee /tmp/htlc_output.txt
