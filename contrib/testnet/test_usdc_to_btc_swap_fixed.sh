...
# BTC balances (FIXED: Use ~/.bitcoin instead of ~/.bitcoin-signet)
echo "BTC:"
...
# Get user's BTC address (FIXED: Use ~/.bitcoin)
...
if [ -n "$BTC_HTLC_ADDR" ]; then
    ...
    echo -e "${YELLOW}! BTC HTLC creation failed${NC}"
    echo "  Response: $BTC_HTLC"
fi
echo ""
...
echo ""
echo -e "${YELLOW}! Skipping actual BTC claim (requires manual Bitcoin scripting)${NC}"
echo "  Secret is known: $SECRET"
echo ""
