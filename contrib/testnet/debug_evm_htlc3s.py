#!/usr/bin/env python3
"""
Debug EVM HTLC3S create function to identify revert reason.
"""
import json
import os
import time
...
# Read saved hashlocks
def read_hashlock(path):
    with open(path) as f:
        return f.read().strip()
...
def main():
    ...
    print(f"Block timestamp: {w3.eth.get_block('latest')['timestamp']}")
    print(f"Local time: {int(time.time())}")
    print()

    # Read hashlocks
    state_dir = "/tmp/flowswap_e2e_state"
    H_user = read_hashlock(f"{state_dir}/H_user")
    H_lp1 = read_hashlock(f"{state_dir}/H_lp1")
    H_lp2 = read_hashlock(f"{state_dir}/H_lp2")

    print(f"H_user: {H_user} (len={len(H_user)})")
    print(f"H_lp1:  {H_lp1} (len={len(H_lp1)})")
    print(f"H_lp2:  {H_lp2} (len={len(H_lp2)})")
    print()
    ...
    print(f"h_user_bytes: {h_user_bytes.hex()} (len={len(h_user_bytes)})")
    print(f"h_lp1_bytes:  {h_lp1_bytes.hex()} (len={len(h_lp1_bytes)})")
    print(f"h_lp2_bytes:  {h_lp2_bytes.hex()} (len={len(h_lp2_bytes)})")
    print()
        ...
        for log in receipt['logs']:
            if log['address'].lower() == HTLC3S_ADDRESS.lower():
                if len(log['topics']) >= 2:
                    ...
                    # Save htlc_id to state
                    with open(f"{state_dir}/htlc_usdc_id", "w") as f:
                        f.write(f"0x{htlc_id}\n")
                    print(f"Saved htlc_id to {state_dir}/htlc_usdc_id")
                    break
    else:
        print("\n✗ Transaction failed!")
