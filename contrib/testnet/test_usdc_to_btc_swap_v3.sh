...
# Correct datadirs for each node
OP1_DATADIR="$HOME/.bitcoin"  # LP uses ~/.bitcoin
OP3_DATADIR="$HOME/.bitcoin-signet"  # User uses ~/.bitcoin-signet
...
# BTC balances (use correct datadirs)
echo "BTC:"
...
# Create HTLC (approval handled in SDK)
...
# Get user's BTC address (correct datadir)
...
USER_BTC_FINAL=$(ssh -i $SSH_KEY ubuntu@$OP3_IP '~/bitcoin/bin/bitcoin-cli -signet -datadir=$HOME/.bitcoin-signet getbalance 2>/dev/null || echo "0"')
echo "BTC:"
