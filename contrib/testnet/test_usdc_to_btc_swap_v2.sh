...
# BTC balances
echo "BTC:"
LP_BTC=$(ssh -i $SSH_KEY ubuntu@$OP1_IP '~/bitcoin/bin/bitcoin-cli -signet -datadir=$HOME/.bitcoin getbalance 2>/dev/null || echo "0"')
USER_BTC=$(ssh -i $SSH_KEY ubuntu@$OP3_IP '~/bitcoin/bin/bitcoin-cli -signet -datadir=$HOME/.bitcoin getbalance 2>/dev/null || echo "0"')
echo "  LP:   $LP_BTC BTC"
...
# Create HTLC directly (approval handled in SDK)
...
~/bitcoin/bin/bitcoin-cli -signet -datadir=$HOME/.bitcoin getnewaddress "swap_receive" bech32
')
...
# Get LP refund address
LP_BTC_REFUND=$(ssh -i $SSH_KEY ubuntu@$OP1_IP '
~/bitcoin/bin/bitcoin-cli -signet -datadir=$HOME/.bitcoin getnewaddress "lp_refund" bech32
')

# Use query parameters
BTC_HTLC=$(curl -s -X POST "$LP_API/api/sdk/btc/htlc/create?amount_sats=$BTC_SATS&hashlock=$HASHLOCK&recipient_address=$USER_BTC_ADDR&refund_address=$LP_BTC_REFUND&timeout_blocks=144" 2>/dev/null)

echo "$BTC_HTLC" | python3 -m json.tool 2>/dev/null | head -15

BTC_HTLC_ADDR=$(echo "$BTC_HTLC" | python3 -c "import json,sys; print(json.load(sys.stdin).get('htlc_address',''))" 2>/dev/null)

if [ -n "$BTC_HTLC_ADDR" ] && [ "$BTC_HTLC_ADDR" != "null" ]; then
    ...
    BTC_FUND_TX=$(ssh -i $SSH_KEY ubuntu@$OP1_IP "
        ~/bitcoin/bin/bitcoin-cli -signet -datadir=\$HOME/.bitcoin \
        sendtoaddress '$BTC_HTLC_ADDR' $BTC_AMOUNT
    " 2>&1)
    
    if [[ "$BTC_FUND_TX" =~ ^[a-f0-9]{64}$ ]]; then
        echo -e "${GREEN}✓ BTC HTLC Funded: $BTC_FUND_TX${NC}"
        echo "  Waiting for confirmation (10s)..."
        sleep 10
    else
        echo -e "${YELLOW}! BTC funding failed: $BTC_FUND_TX${NC}"
    fi
else
    echo -e "${YELLOW}! BTC HTLC creation failed or not available${NC}"
    echo "  (This is expected - BTC HTLC creation requires Bitcoin Script support)"
fi
echo ""
...
echo ""
echo -e "${YELLOW}! Skipping actual BTC claim (requires Bitcoin Script implementation)${NC}"
...
echo "LP claims USDC HTLC with secret..."
...
LP_BTC_FINAL=$(ssh -i $SSH_KEY ubuntu@$OP1_IP '~/bitcoin/bin/bitcoin-cli -signet -datadir=$HOME/.bitcoin getbalance 2>/dev/null || echo "0"')
USER_BTC_FINAL=$(ssh -i $SSH_KEY ubuntu@$OP3_IP '~/bitcoin/bin/bitcoin-cli -signet -datadir=$HOME/.bitcoin getbalance 2>/dev/null || echo "0"')
echo "BTC:"
echo "  LP:   $LP_BTC_FINAL BTC"
echo "  User: $USER_BTC_FINAL BTC"
...
echo -e "${MAGENTA}║  User would receive: $BTC_AMOUNT BTC (pending claim)           ║${NC}"
echo -e "${MAGENTA}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
