...
#   - genesis_burns.json must exist
#   - BTC Signet must be reachable
#   - SPV tip must be >= max burn height
...
#   - OP2 (57.131.33.214): Peer only
#   - OP3 (51.75.31.44): Peer only
# Uses keys from ~/.pivkey/operator_keys.json on Seed
# ==============================================================================
restore_mn_configs() {
    log "Restoring MN configs (v13.0 burn-based: 1 operator, 8 MNs on Seed)..."

    # Try to read operator keys from Seed's ~/.pivkey/operator_keys.json
    # These keys are generated by bootstrap_testnet.sh and match the ProRegTx
    local KEYS_JSON=$($SSH ubuntu@57.131.33.151 "cat ~/.pivkey/operator_keys.json 2>/dev/null" 2>/dev/null)

    if [ -n "$KEYS_JSON" ]; then
        log "  Using operator keys from Seed (~/.pivkey/operator_keys.json)"
...
EOF
    ...
    # Restore SPV backup if exists (fast path — avoids slow header-by-header sync)
    local has_backup=$($SSH ubuntu@$SEED_IP 'test -f ~/btcspv_backup_latest.tar.gz && echo "yes" || echo "no"')
    if [ "$has_backup" = "yes" ]; then
        log "  Restoring SPV backup..."
        $SSH ubuntu@$SEED_IP "
    ...
    # Check if restored backup is sufficient (tip >= btc_tip)
    # Kill any leftover daemon first, then start temp daemon to query SPV tip
    $SSH ubuntu@$SEED_IP 'pkill -9 bathrond 2>/dev/null || true; sleep 2; rm -f ~/.bathron/testnet5/.lock' 2>/dev/null
    ...
    $SSH ubuntu@$SEED_IP "
        BTCCLI=~/bitcoin-27.0/bin/bitcoin-cli
        BATHCLI=~/bathron-cli
        CURRENT=$((spv_tip + 1))
        TARGET=$btc_tip
        BATCH=500
        ...
        while [ \$CURRENT -le \$TARGET ]; do
            ...
            for h in \$(seq \$CURRENT \$END); do
                hash=\$(\$BTCCLI -datadir=~/.bitcoin-signet getblockhash \$h 2>/dev/null)
                header=\$(\$BTCCLI -datadir=~/.bitcoin-signet getblockheader \$hash false 2>/dev/null)
                headers_hex=\"\${headers_hex}\${header}\"
            done
            \$BATHCLI -testnet submitbtcheaders \"\$headers_hex\" >/dev/null 2>&1
            DONE=\$((END - $spv_tip))
            PCT=\$((DONE * 100 / TOTAL))
            echo \"Progress: \${PCT}% (\${DONE}/\${TOTAL})\"
            CURRENT=\$((END + 1))
        done
...
create_genesis_bootstrap() {
    ...
    log "  DAEMON-ONLY FLOW (no genesis_burns*.json):"
    log "    Block 1: TX_BTC_HEADERS only"
    log "    Burns detected LIVE from BTC Signet by btc_burn_claim_daemon"
    log "    K=20 blocks → TX_MINT_M0BTC → MN registration"

    local KEYS_FILE="$HOME/.pivkey/testnet_keys.json"
    local BURN_KEYS_FILE="$HOME/.pivkey/burn_dest_keys.json"
    ...
    local BURN_SIMPLE="$HOME/BATHRON/contrib/testnet/genesis_claim_burns_simple.sh"
    local GENESIS_BURNS="$HOME/BATHRON/contrib/testnet/genesis_burns.json"
    ...
    # CRITICAL: genesis_claim_burns_simple.sh is called by genesis_bootstrap_seed.sh
    $SCP "$BURN_SIMPLE" ubuntu@$SEED_IP:~/genesis_claim_burns_simple.sh 2>/dev/null
    # CRITICAL: genesis_burns.json for FAST burn claiming (no scan needed)
    [ -f "$GENESIS_BURNS" ] && $SCP "$GENESIS_BURNS" ubuntu@$SEED_IP:~/genesis_burns.json 2>/dev/null
    $SSH ubuntu@$SEED_IP "chmod +x ~/genesis_claim_burns_simple.sh ~/btc_burn_claim_daemon.sh" 2>/dev/null
    ...
    $SSH ubuntu@$SEED_IP 'chmod +x /tmp/genesis_bootstrap_seed.sh && nohup /tmp/genesis_bootstrap_seed.sh > /tmp/genesis_bootstrap.log 2>&1 & echo $!'
    ...
    local MAX_WAIT=900  # 15 minutes (increased from 10 for slow BTC scans)
    local WAITED=0
    ...
    while [ $WAITED -lt $MAX_WAIT ]; do
        ...
        log "  ...waited ${WAITED}s/${MAX_WAIT}s, bootstrap height: $HEIGHT"

        if [ "$HEIGHT" -ge 22 ]; then
            log "Bootstrap reached sufficient height ($HEIGHT)"
            BOOTSTRAP_EXIT=0
            break
        fi
    done
...
copy_binaries() {
    ...
    # Step 4: OP1 special - also copy to pna-lp path for LP Dashboard
...
copy_binaries_to_seed() {
    ...
    # Ensure executable and verify
    $SSH ubuntu@$ip "
        chmod +x ~/BATHRON-Core/src/bathrond ~/BATHRON-Core/src/bathron-cli
        if [ ! -x ~/BATHRON-Core/src/bathrond ] || [ ! -x ~/BATHRON-Core/src/bathron-cli ]; then
            echo 'ERROR: Binaries not executable!'
            exit 1
        fi
...
genesis_step_1_spv_prepare() {
    ...
    # GATE: SPV backup must exist on Seed (no daemon running yet to query RPC)
    if ! $DRY_RUN; then
        local spv_exists=$($SSH ubuntu@$SEED_IP "test -d $VPS_TESTNET_DIR/btcspv && ls $VPS_TESTNET_DIR/btcspv/*.ldb 2>/dev/null | wc -l" 2>/dev/null || echo "0")
        if [ "$spv_exists" -lt 1 ]; then
            error "GATE FAILED: btcspv/ not found or empty on Seed"
            error "  Ensure SPV backup exists at ~/btcspv_backup_latest.tar.gz"
            exit 1
        fi
        success "GATE OK: btcspv/ exists on Seed ($spv_exists ldb files)"
    fi

    success "STEP 1 COMPLETE: SPV prepared on Seed"
}

# Genesis Step 1b: Refresh genesis_burns.json from BTC Signet (INCREMENTAL SCAN)
# Only scans NEW blocks since last known burn - much faster than full rescan
genesis_step_refresh_burns() {
    echo ""
    log "═══════════════════════════════════════════════════════════════"
    log "GENESIS STEP 1b: Refresh genesis_burns.json from BTC Signet"
    log "═══════════════════════════════════════════════════════════════"

    if $DRY_RUN; then
        log "[DRY-RUN] Would scan BTC Signet for burns"
        return 0
    fi
    ...
    local BURNS_FILE="$SCRIPT_DIR/genesis_burns.json"
    local BATHRON_MAGIC="42415448524f4e"

    # Get BTC tip
    local btc_tip=$($SSH ubuntu@$SEED_IP '$HOME/bitcoin-27.0/bin/bitcoin-cli -datadir=$HOME/.bitcoin-signet getblockcount' 2>/dev/null)
    if [ -z "$btc_tip" ] || [ "$btc_tip" -lt "$BTC_CHECKPOINT" ]; then
        error "Cannot reach BTC Signet or tip too low (tip=$btc_tip)"
        return 1
    fi
    local safe_height=$((btc_tip - K_CONF))

    # Check if genesis_burns.json exists and determine scan start
    local scan_start=$BTC_CHECKPOINT
    local existing_burns=""
    if [ -f "$BURNS_FILE" ]; then
        local max_height=$(jq -r '[.burns[].btc_height] | max // 0' "$BURNS_FILE" 2>/dev/null)
        if [ -n "$max_height" ] && [ "$max_height" -gt 0 ]; then
            scan_start=$((max_height + 1))
            existing_burns=$(jq -c '.burns' "$BURNS_FILE" 2>/dev/null)
            log "  Existing burns up to height $max_height ($(jq '.burns | length' "$BURNS_FILE") burns)"
        fi
    fi

    # If already up to date, skip scan
    if [ "$scan_start" -gt "$safe_height" ]; then
        local burn_count=$(jq '.burns | length' "$BURNS_FILE" 2>/dev/null)
        local burn_total=$(jq '[.burns[].amount] | add' "$BURNS_FILE" 2>/dev/null)
        log "  Already up to date (last scan: $((scan_start - 1)), tip: $safe_height)"
        success "STEP 1b COMPLETE: genesis_burns.json current ($burn_count burns, $burn_total sats)"
        $SCP "$BURNS_FILE" ubuntu@$SEED_IP:~/genesis_burns.json 2>/dev/null
        return 0
    fi

    log "  BTC tip: $btc_tip, scanning $scan_start to $safe_height ($(($safe_height - $scan_start + 1)) blocks)..."

    # Scan for NEW burns only
    local new_burns=$($SSH ubuntu@$SEED_IP "bash -s" << INNER_EOF
BTC="\$HOME/bitcoin-27.0/bin/bitcoin-cli -datadir=\$HOME/.bitcoin-signet"
MAGIC="$BATHRON_MAGIC"
START=$scan_start
END=$safe_height
...
for HEIGHT in \$(seq \$START \$END); do
    HASH=\$(\$BTC getblockhash \$HEIGHT 2>/dev/null)
    [ -z "\$HASH" ] && continue

    TXIDS=\$(\$BTC getblock "\$HASH" 1 2>/dev/null | jq -r '.tx[]')
    for TXID in \$TXIDS; do
        RAW=\$(\$BTC getrawtransaction "\$TXID" 2>/dev/null)
        [ -z "\$RAW" ] && continue

        if echo "\$RAW" | grep -qi "\$MAGIC"; then
            TX=\$(\$BTC getrawtransaction "\$TXID" true 2>/dev/null)
            AMOUNT=\$(echo "\$TX" | jq -r '.vout[] | select(.scriptPubKey.type == "witness_v0_scripthash") | .value' 2>/dev/null | head -1)

            if [ -n "\$AMOUNT" ] && [ "\$AMOUNT" != "null" ]; then
                SATS=\$(echo "\$AMOUNT * 100000000" | bc | cut -d. -f1)
                [ \$FIRST -eq 0 ] && echo ','
                FIRST=0
                echo "{\"btc_txid\": \"\$TXID\", \"btc_height\": \$HEIGHT, \"amount\": \$SATS}"
            fi
        fi
    done
    [ \$((HEIGHT % 100)) -eq 0 ] && echo "  ... scanned to \$HEIGHT" >&2
done
INNER_EOF
    )

    # Merge existing + new burns
    local final_json
    if [ -n "$existing_burns" ] && [ "$existing_burns" != "[]" ] && [ "$existing_burns" != "null" ]; then
        if [ -n "$new_burns" ]; then
            # Merge: existing + new
            final_json=$(echo "{\"burns\": $existing_burns}" | jq --argjson new "[$new_burns]" '.burns += $new | .burns |= sort_by(.btc_height)')
            log "  Found $(echo "[$new_burns]" | jq 'length') new burns"
        else
            final_json="{\"burns\": $existing_burns}"
            log "  No new burns found"
        fi
    else
        if [ -n "$new_burns" ]; then
            final_json="{\"burns\": [$new_burns]}"
        else
            final_json='{"burns": []}'
        fi
    fi
    ...
    # Validate and save
    if ! echo "$final_json" | jq -e '.burns' > /dev/null 2>&1; then
        error "Failed to parse merged burns JSON"
        return 1
    fi

    local burn_count=$(echo "$final_json" | jq '.burns | length')
    local burn_total=$(echo "$final_json" | jq '[.burns[].amount] | add')

    echo "$final_json" | jq '.' > "$BURNS_FILE"
    success "STEP 1b COMPLETE: genesis_burns.json updated ($burn_count burns, $burn_total sats)"

    $SCP "$BURNS_FILE" ubuntu@$SEED_IP:~/genesis_burns.json 2>/dev/null
    log "  Copied to Seed: ~/genesis_burns.json"
}

# Genesis Step 2: Create bootstrap blocks 0-3
# Genesis Step 1c: Backup burn destination private keys from Seed
# The burn destinations in OP_RETURN may reference addresses from a previous wallet.
# We dump those keys before the wipe so they can be re-imported after genesis.
genesis_step_1c_backup_burn_keys() {
    echo ""
    log "═══════════════════════════════════════════════════════════════"
    log "GENESIS STEP 1c: Backup burn destination keys from Seed"
    log "═══════════════════════════════════════════════════════════════"

    if $DRY_RUN; then
        log "[DRY-RUN] Would backup burn destination keys from Seed wallet"
        return 0
    fi

    # DAEMON-ONLY FLOW: Use genesis_burns.json for destination addresses (simpler file)
    local GENESIS_BURNS="$HOME/BATHRON/contrib/testnet/genesis_burns.json"
    if [ ! -f "$GENESIS_BURNS" ]; then
        warn "  No genesis_burns.json found, skipping key backup"
        warn "  (Keys can be imported later if burn destinations are known)"
        return 0
    fi

    # Extract unique burn destinations
    local DESTS=$(jq -r '[.burns[].bathron_dest] | unique | .[]' "$GENESIS_BURNS" 2>/dev/null)
    local DEST_COUNT=$(echo "$DESTS" | wc -l)
    log "  Unique burn destinations: $DEST_COUNT"

    local BURN_KEYS_FILE="$HOME/.pivkey/burn_dest_keys.json"

    # Try to dump keys from Seed main daemon
    local MAIN_CLI="bathron-cli -testnet"
    local KEYS_DUMPED=0
    local KEYS_JSON="{"

    # Check if Seed daemon is running
    local SEED_RUNNING=$($SSH ubuntu@$SEED_IP "pgrep bathrond >/dev/null 2>&1 && echo yes || echo no" 2>/dev/null)

    if [ "$SEED_RUNNING" = "yes" ]; then
        log "  Seed daemon running, dumping burn destination keys..."
        for DEST in $DESTS; do
            local WIF=$($SSH ubuntu@$SEED_IP "$MAIN_CLI dumpprivkey $DEST 2>/dev/null" 2>/dev/null || echo "")
            if [ -n "$WIF" ] && [ "$WIF" != "null" ]; then
                [ $KEYS_DUMPED -gt 0 ] && KEYS_JSON+=","
                KEYS_JSON+="\"$DEST\":\"$WIF\""
                KEYS_DUMPED=$((KEYS_DUMPED + 1))
            fi
        done
    else
        log "  Seed daemon not running, checking existing backup..."
    fi

    KEYS_JSON+="}"

    if [ $KEYS_DUMPED -gt 0 ]; then
        echo "$KEYS_JSON" | jq '.' > "$BURN_KEYS_FILE"
        success "  Dumped $KEYS_DUMPED/$DEST_COUNT burn destination keys to $BURN_KEYS_FILE"
    elif [ -f "$BURN_KEYS_FILE" ]; then
        local EXISTING=$(jq 'length' "$BURN_KEYS_FILE" 2>/dev/null || echo "0")
        warn "  Using existing backup ($EXISTING keys from previous dump)"
    else
        warn "  ══════════════════════════════════════════════════════════════"
        warn "  NO BURN DESTINATION KEYS AVAILABLE!"
        warn "  The wallet with burn destination keys has been wiped."
        warn "  Create ~/.pivkey/burn_dest_keys.json with format:"
        warn "    {\"<address>\": \"<WIF>\", ...}"
        warn "  Or re-burn to known addresses (alice, bob, etc.)"
        warn "  ══════════════════════════════════════════════════════════════"
    fi

    success "STEP 1c COMPLETE: Key backup processed"
}
...
genesis_step_2_create() {
    ...
    if ! $DRY_RUN; then
        gate_ssh_ok "$SEED_IP" "test -f ~/.pivkey/operator_keys.json" "operator_keys.json exists"
    fi
...
genesis_step_3_configure() {
    ...
    if ! $DRY_RUN; then
        scp -i "$SSH_KEY" -o StrictHostKeyChecking=no ubuntu@$SEED_IP:~/.pivkey/operator_keys.json /tmp/mn_operator_keys.json 2>/dev/null
...
genesis_step_4_distribute() {
    ...
    # Distribute to all nodes (including Seed - uniform treatment)
    ...
    for ip in "${VPS_NODES[@]}"; do
        (
            ...
            if ! $SCP /tmp/testnet5_bootstrap.tar.gz ubuntu@$ip:/tmp/; then
                echo "DIST_FAIL:$ip:scp"
                exit 1
            fi

            # Wipe and extract
            if ! $SSH ubuntu@$ip "
                # Stop any daemon that might have restarted
                pkill -9 bathrond 2>/dev/null || true
                sleep 1
                ...
                # COMPLETE WIPE of testnet5 (uniform for all nodes)
                rm -rf $VPS_TESTNET_DIR

                # Extract fresh chain data
...
genesis_step_6_verify() {
    ...
    log "Waiting 90s for block production and sync..."
    sleep 90
    ...
    # GATE 3: Same bestblockhash on all nodes (consensus check)
    log "Checking consensus (bestblockhash)..."
    local ref_hash=""
    for ip in "${VPS_NODES[@]}"; do
        local cli_path=$(get_cli_path "$ip")
        local hash=$($SSH ubuntu@$ip "$cli_path -testnet getbestblockhash 2>/dev/null" 2>/dev/null || echo "")
        if [ -z "$ref_hash" ]; then
            ref_hash=$hash
        elif [ "$hash" != "$ref_hash" ]; then
            error "GATE FAILED: Consensus mismatch!"
            error "  $SEED_IP: ${ref_hash:0:16}..."
            error "  $ip: ${hash:0:16}..."
            exit 1
        fi
    done
    success "GATE OK: All nodes on same chain (${ref_hash:0:16}...)"
...
detect_genesis_state() {
    ...
    if $SSH ubuntu@$SEED_IP "test -f ~/.pivkey/operator_keys.json" 2>/dev/null; then
        detected_step=3
...
genesis_full() {
    ...
    [ "$RESUME_FROM" -le 1 ] && genesis_step_refresh_burns
    [ "$RESUME_FROM" -le 1 ] && genesis_step_1c_backup_burn_keys
...
if $SPV_VERIFY; then
    ...
    local GENESIS_BURNS="$HOME/BATHRON/contrib/testnet/genesis_burns.json"
    if [ ! -f "$GENESIS_BURNS" ]; then
        error "genesis_burns.json not found!"
        exit 1
    fi

    local H_REQUIRED=$(jq '[.burns[].btc_height] | max' "$GENESIS_BURNS")
    log "H_required (max burn height): $H_REQUIRED"
    echo ""
