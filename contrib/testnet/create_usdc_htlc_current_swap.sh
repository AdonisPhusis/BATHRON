...
# Script: Create USDC HTLC for current atomic swap
...
cat > /tmp/create_usdc_htlc_current.py << 'PYTHON_EOF'
#!/usr/bin/env python3
"""
Create USDC HTLC on Base Sepolia - Current swap version.
...
scp -i ~/.ssh/id_ed25519_vps /tmp/create_usdc_htlc_current.py ubuntu@51.75.31.44:/tmp/

echo "Executing HTLC creation on OP3..."
ssh -i ~/.ssh/id_ed25519_vps ubuntu@51.75.31.44 \
  "SWAP_ID='$SWAP_ID' HASHLOCK='$HASHLOCK' HTLC_CONTRACT='$HTLC_CONTRACT' \
   RECEIVER='$RECEIVER' TOKEN='$TOKEN' AMOUNT_USDC='$AMOUNT_USDC' \
   TIMELOCK_SECONDS='$TIMELOCK_SECONDS' \
   python3 /tmp/create_usdc_htlc_current.py" | tee /tmp/htlc_output.txt
...
if grep -q "HTLC_ID=" /tmp/htlc_output.txt; then
    ...
    echo ""
    echo "Waiting 10 seconds for LP to process..."
    sleep 10
