...
declare -A REQUIRED_PATTERNS=(
    ...
    # Settlement Layer (M0/M1)
    ["TX_LOCK"]="LOCK transaction type"
    ["TX_UNLOCK"]="UNLOCK transaction type"
    ["TX_TRANSFER_M1"]="M1 transfer transaction type"

    # Settlement RPC Commands
    ["\"lock\""]="Lock RPC command"
    ["\"unlock\""]="Unlock RPC command"
    ["\"transfer_m1\""]="Transfer M1 RPC command"
    ["\"getstate\""]="Get settlement state RPC"
    ["\"listreceipts\""]="List M1 receipts RPC"
...
# ═══════════════════════════════════════════════════════════════════════════════
# SAPLING / zKPIV VERIFICATION
# ═══════════════════════════════════════════════════════════════════════════════

verify_sapling() {
    print_section "Verifying Sapling/zKPIV components"
    ...
    # Check for zKPIV-related code
    local zkhu_refs=$(grep -rn "zKPIV\|KPIV_STAKE" "$REPO_ROOT/src" --include="*.cpp" --include="*.h" 2>/dev/null | wc -l)
    if [ "$zkhu_refs" -gt 0 ]; then
        print_ok "zKPIV/KPIV_STAKE references found ($zkhu_refs occurrences)"
    else
        print_warn "No zKPIV/KPIV_STAKE references found"
    fi
}
